import { costPerKid, formatCurrency, formatHorizon, listingProofRows, listingTrustBadges } from "./data/listings";
...
export function TrustBadgeRow({ listing }: { listing: Listing }) {
  const badges = listingTrustBadges(listing);
  if (badges.length === 0) return null;
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {badges.map((b) => {
        const s = BADGE_STYLE[b.style];
        const extra = b.style === "example" ? "border border-slate-200 font-semibold tracking-wide" : "";
        return (
          <span
            key={`${b.style}-${b.label}`}
...
export function ListingCard({ listing, onClick }: { listing: Listing; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="font-semibold text-slate-900 text-[15px] leading-snug mt-2.5">{listing.title}</h3>
      {listing.organization && (
        <p className="text-sm text-slate-500 mt-0.5">
          {listing.organization} · {listing.metro}, {listing.state}
        </p>
      )}
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs text-slate-500">
        ...
        <span className="inline-flex items-center gap-1"><Users className="w-3.5 h-3.5" />{listing.kidsServed.toLocaleString()} children · {formatHorizon(listing)} · {listing.modelType}</span>
      </div>
